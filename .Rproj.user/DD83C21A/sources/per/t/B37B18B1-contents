...
tc_clusters_df <- tc_clusters_df %>% 
  mutate(traj_titles  = ifelse(test = traj == "A", yes = "N = 23,924 (72%)", no = traj),
         traj_titles  = ifelse(test = traj == "B", yes = "N = 8,124 (26%)" , no = traj_titles),
         traj_titles  = ifelse(test = traj == "C", yes = "N = 5,664 (3%)"  , no = traj_titles),
...
# Check.
table(tc_clusters_df$traj_titles)
...
march = "march",# We know that the most change occurred in April, across all crimes types.
