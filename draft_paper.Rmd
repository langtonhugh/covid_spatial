...
This paper has aimed to investigate the localized drivers of the COVID-19 crime drop in England and Wales. Using 7-months of police-recorded data on notifiable offences (excluding drugs), we decomposed the (macro) nationwide decline in crime according to clusters of (meso) local areas, each with distinct longitudinal trajectories. Findings provide substantial evidence to suggest that the nationwide decline in crime during lockdown has been driven by a disproportionately small number of localized geographic areas. We find that these localized drivers, dominating city centers, had pre-existing high levels of crime attributable to their unique opportunity structures, containing numerous nightlife facilities, commercial shops, public transport nodes and bicycle parking spaces. Clusters demonstrate a degree of stability in their crime type profiles, but the 'high crime major drop' areas appear to be weighted towards theft and shoplifting. These areas experienced a fairly slow resurgence upon the relaxation of lockdown rules, remaining below historical levels even by the time low and mid-crime clusters had returned to normality.

The finding that a small number of fine-grained geographic units drove the macro-level crime drop, with most areas otherwise remaining fairly stable, is consistent with results from other study sites. Evidence from both Australia [@payneregional2021] and the United States [@campedelli2020disentangling] suggests that the lockdown crime drop was not uniform, but instead, the macro-level trend masked underlying variation.

Here, we have quantified and described this localized variation for both the decline and resurgence stages of the pandemic.



is also consistent with findings from the pre-pandemic era, in which


we would argue that this study has extended these existing findings in three key areas. Firstly, we have examined both the lock


The non-parametric longitudinal clustering technique deployed here has permitted, for the first time, a comprehensive evaluation of this using fine-grained spatial scales in England and Wales.


 [@andresen2017trajectories; @weisburd2012criminology; @mcvie2020increasing].
