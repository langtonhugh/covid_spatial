...
- Here, we provide a background of nationwide trends by crime type (counts, Gini) before proceeding with a broader 'notifiable offences (excluding drugs)' category.
...
- Due to some LSOA not having snap points within them, we end up with a final population size of N = 33,075.
...
-  We accept that granularity is lost with examining notifiable offences in their totality. We address this using a crime type breakdown, and return to it in the more detail for the discussion.
...
- We note from the Gini coefficient that most crime types experienced remarkably stable 'global' concentration, even amidst dramatic volatility in crime counts.
...
- Overall, however, notifiable offences (excluding drugs) experienced minimal change. There is a marginal dip in concentration during the first two months of lockdown, quickly returning to normal levels of concentration as crime surged back upwards during summer.
...
- What is the crime type profile of the area driving nationwide trends?
- We can answer these questions using k-means to cluster the longitudinal trajectories of LSOA.
...
- Violin plots have been added to convey the variation in counts at each time point. Here, we note that distributions were fairly uniform, with the mean and median representing the cluster solutions fairly well, and little overlap between clusters.
- For reference to what we trends we may have expected in a 'typical' year without the pandemic, we have added the mean and median trend lines for each cluster during 2018 (in blue) and 2019 (in red).
...
- Interestingly, clusters E and F represent the only clusters which did not return back to normality by August, even we assume that 2018 and 2019 represent a trend which we might have otherwise expected.
...
- Alongside these raw counts, we also include the percentage of total absolute change (i.e. positive or negative) nationwide attributable to each cluster.
...
- July to August represents an interesting period. Until that point, each cluster, despite their vastly differing relative contributions to change, with directionally homogeneous. In other words, every cluster declines between February and April, and then increased between April and July.
