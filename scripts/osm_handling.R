...
# Note that this script has to be run at stages due to the limits on the OSM API. Each query is coded
# separately.
...
# data, some LSOAs had upto ~400 pubs, but most were not named. If they have no name, we doubt
...
# Bicycle parking.
bike_sf <- opq(bbox = ew_bb, timeout = 100) %>%
  add_osm_feature(key = "amenity", value = "bicycle_parking") %>% 
  osmdata_sf()

# Points.
bike_pts_sf <- bike_sf$osm_points

# Deplicates (a small number).
length(unique(bike_pts_sf$geometry)) 

# Complete.
bike_pts_complete_sf <- bike_pts_sf %>% 
  distinct(geometry)

# Project
bike_pts_complete_sf <- bike_pts_complete_sf %>% 
  st_transform(crs = 27700)

# Aggregate bar points to LSOA.
lsoa_ew_valid_sf <- lsoa_ew_valid_sf %>% 
  mutate(bikes = lengths(st_intersects(lsoa_ew_valid_sf, bike_pts_complete_sf)))

# Car parking spaces.
car_space_sf <- opq(bbox = ew_bb, timeout = 100) %>%
  add_osm_feature(key = "amenity", value = "parking_space") %>% 
  osmdata_sf()

# Points.
car_space_pts_sf <- car_space_sf$osm_points

# Deplicates (a small number).
length(unique(car_space_pts_sf$geometry)) 

# Complete.
car_space_pts_complete_sf <- car_space_pts_sf %>% 
  distinct(geometry)

# Project
car_space_pts_complete_sf <- car_space_pts_complete_sf %>% 
  st_transform(crs = 27700)

# Aggregate bar points to LSOA.
lsoa_ew_valid_sf <- lsoa_ew_valid_sf %>% 
  mutate(car_spaces = lengths(st_intersects(lsoa_ew_valid_sf, car_space_pts_complete_sf)))
...
# Car parks. OSM specifies these should not overlap with the above. We choose the entrance because
# otherwise we get polygons which are far too big in size nationwide.
car_parking_sf <- opq(bbox = ew_bb, timeout = 100) %>%
  add_osm_feature(key = "amenity", value = "parking_entrance") %>% 
  osmdata_sf()

# Points.
car_parking_pts_sf <- car_parking_sf$osm_points

# Deplicates,
length(unique(car_parking_pts_sf$geometry)) 

# Complete.
car_parking_pts_complete_sf <- car_parking_pts_sf %>% 
  distinct(geometry)

# Project
car_parking_pts_complete_sf <- car_parking_pts_complete_sf %>% 
  st_transform(crs = 27700)

# Aggregate bar points to LSOA.
lsoa_ew_valid_sf <- lsoa_ew_valid_sf %>% 
  mutate(car_parkings = lengths(st_intersects(lsoa_ew_valid_sf, car_parking_pts_complete_sf)))
...
moto_parking_sf <- opq(bbox = ew_bb, timeout = 300) %>%
  add_osm_feature(key = "amenity", value = "motorcycle_parking") %>% 
  osmdata_sf()

# Points.
moto_parking_pts_sf <- moto_parking_sf$osm_points

# Deplicates,
length(unique(moto_parking_pts_sf$geometry)) 

# Complete.
moto_parking_pts_complete_sf <- moto_parking_pts_sf %>% 
  distinct(geometry)

# Project
moto_parking_pts_complete_sf <- moto_parking_pts_complete_sf %>% 
  st_transform(crs = 27700)

# Aggregate bar points to LSOA.
lsoa_ew_valid_sf <- lsoa_ew_valid_sf %>% 
  mutate(moto_parkings = lengths(st_intersects(lsoa_ew_valid_sf, moto_parking_pts_complete_sf)))
