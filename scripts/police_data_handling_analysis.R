...
# Explore these missings - run for each year. Crimes with missing LSOA appear to
...
# Choose array of possible clusters. We know retrospectively that 6 was optimal.
# n <- 3:6 
# n <- 3:10 
n <- 6
...
kml(tc_traj, nbClusters = n, toPlot = "none", nbRedrawing = 20)

# For Criterion plotting (slower) and save.
...
# Cluster investigation. Note this required a multiple cluster solution as above.
...
kmeans_violin_full_gg <- ggplot() + theme_bw() +
  ...
  stat_summary(data = filter(tc_clusters_agg_df, year == "2018"),
               mapping = aes(x = month_name, y = ew_crime_count, group = traj_titles, colour = year),
  ...
  stat_summary(data = filter(tc_clusters_agg_df, year == "2020"),
               mapping = aes(x = month_name, y = ew_crime_count, group = traj_titles, colour = year),
               fun = "mean", linetype = "dotted", size = 0.8, geom = "line") +
  scale_colour_manual(values = c("blue", "red", "black"), labels = c("2018", "2019", "2020")) +
...
# Calculate the absolute % of month-on-month change each cluster contributed to. This is a bit fiddly and
# could be improved in terms of code.
...
# Save.
ggsave(plot = traj_crimes_gg, filename = "visuals/traj_crimes_monthly_k6_gg.png", width = 16, height = 8, unit = "cm")
...
# Save.
ggsave(plot = heatmaps_gg, filename = "visuals/heatmaps_gg.png", width = 16, height = 6, unit = "cm")
...
# Add OSM data to define city centres. This has been previsously pulled via the osmdata package.
osm_df <- read_csv("data/osm_full.csv")

# Create additional sums required (see below for actual OSM statistics by cluster).
osm_df <- osm_df %>% 
...
# Load (back) in LSOA-level facilities down OSM (see osm_handling.r).
osm_df <- read_csv("data/osm_full.csv")
