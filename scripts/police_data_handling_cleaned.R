...
# Absolute crime profile plots.

# Subset only the study period February to August 2020, calculate crime type % composition of each cluster.
traj_crime_types_counts_df <- sub_data_agg_full_df %>% 
  filter(crime_type != "Anti-social behaviour" & crime_type != "Drugs",
         month == "2020-02" | month == "2020-03" | month == "2020-04" | month == "2020-05" |
          month == "2020-06" | month == "2020-07" | month == "2020-08") %>% 
  group_by(traj_titles, month, crime_type) %>% 
  summarise(traj_crimes = mean(crime_count)) %>% 
  ungroup() 

# Plot.
traj_crimes_count_gg <- traj_crime_types_counts_df %>% 
  ggplot() +
  geom_line(mapping = aes(x = month, y = traj_crimes, colour = crime_type, group = crime_type)) +
